...
&\quad - \underbrace{h_{\text{wall}} \, A_{\text{wall,upper}} \, (T_{\text{upper}} - T_{\text{wall}})}_{\text{壁面熱損失}} \\
&\quad + \underbrace{Q_{\text{rad}} \, F_{\text{upper}}}_{\text{ヒーター輻射 (fixed wall時)}}
\end{align*}
...
&= \underbrace{Q_{\text{rad}} \times F_{\text{lower}}}_{\text{輻射受熱}} \\
&\quad + \underbrace{k_{\text{int}} \, A_{\text{floor}} \, \frac{T_{\text{upper}} - T_{\text{lower}}}{0.1 H}}_{\text{界面伝導}} \\
&\quad - \underbrace{h_{\text{wall}} \, A_{\text{wall,lower}} \, (T_{\text{lower}} - T_{\text{wall}})}_{\text{壁面熱損失}}
\end{align*}
...
\frac{\bar{T}_{\text{col}} - T_{\text{ambient}}}{\bar{T}_{\text{col}}}
$

\textbf{注:} 分母は $\bar{T}_{\text{col}}$（室内カラム平均温度）である。
理想気体の密度差 $\rho_{\text{amb}} - \bar{\rho}_{\text{col}} = \rho_{\text{amb}}(1 - T_{\text{amb}}/\bar{T}_{\text{col}})$
から導出される。分母を $T_{\text{amb}}$ とすると、サウナのように内外温度差が大きい場合に
駆動圧力が過大評価され、不自然に強力な換気が発生する。
...
$
\dot{m}_{\text{vent}} = \text{sgn}(\Delta p) \; C_d A_{\text{eff}} \sqrt{2 \rho_{\text{upwind}} |\Delta p|}
$

ここで:

\begin{itemize} \item $C_d A_{\text{eff}} = \min(C_{d,s} A_s, \, C_{d,e} A_e)$: 有効開口面積（流量は狭い方で制限） \end{itemize}
\begin{itemize} \item $\rho_{\text{upwind}}$: 風上密度。流入時（$\Delta p > 0$）は $\rho_{\text{amb}}$（外気密度）、
  逆流時（$\Delta p < 0$）は $\rho_{\text{supply}}$（給気口位置の室内密度）を使用 \end{itemize}
\begin{itemize} \item 正（流入）が通常だが、ロウリュ蒸気膨張時の室内圧上昇で負（逆流＝排出）も許容 \end{itemize}
...
&\quad - \underbrace{\frac{\lambda_w}{\delta_w} A_{\text{wall}}
  (T_{\text{wall,inner}} - T_{\text{wall,outer}})}_{\text{壁面→外部熱伝導}}
\end{align*}
